...
readxl::read_excel(here::here("data", "TRIMIS waterborne-organisations 14052025.xlsx"),
    ...
    mutate(activityType = ifelse(eu == "Yes" & is.na(activityType), "OTH", activityType)) |> 
                           ...
                           detail = "")
{
    ...
    if(!(deparse(substitute(detail)) %in% c("topic", "theme")))
...
trimis_eu_participation <-  trimis_participation |> 
    filter(country %in% eu27)

country_participation <- trimis_eu_participation |> 
    ...
    mutate(not_sme = partners - sme,
        sme_share = sme/partners) |> 
    tidyr::replace_na(list(lead = 0, sme = 0, sme_share = 0 )) |> 
    filter(n > 10) 

country_participation|> 
    readr::write_csv(here::here("data", "for-figures", "country_participation.csv"))
    
specialisation <- projects |> 
    group_by(theme) |> 
    summarise(spec_n = n()) |> 
    mutate(n_share_avg = spec_n / nrow(projects))

country_specialisation <- trimis_participation |> 
    filter(country %in% eu27 ) |> 
    group_orgs_cat(category = country, detail = theme) |> 
    filter(country %in% country_participation$country) |> 
    mutate(n_country = sum(n),
           n_share = n / sum(n)) |> 
    left_join(specialisation,
              by = "theme") |> 
    mutate(specialisation = ifelse(n_share > n_share_avg * 1.1 &
                                       n > n_country * n_share_avg + 2,
                                   "yes", "no")) |> 
    arrange(country, desc(spec_n))
    
readr::write_csv(country_specialisation, 
                 here::here("data", "for-figures", "country_specialisation.csv"))
